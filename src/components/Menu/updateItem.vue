        ...
        <div class="card-body">
          <form>
            ...
            <div class="row">
              ...
              <span class="text-danger px-4" v-if="v$.selectCate.$errors[0]">
            ...
            <div class="pt-1 mt-5 d-flex justify-content-center gap-3">
              ...
              <button
                class="btn btn-info px-5 py-2 d-block text-white fw-bold update-btn"
                type="button"
                @click="UpdateItem()"
                style="width: 200px"
              >
...
export default {
  ...
  mounted() {
    ...
    if (!user) {
      ...
      this.getItemInfo(this.restaurantId,this.itemId)
    }
  },
  methods: {
    ...
    async getItemInfo(restId,itemId){
      let result = await axios.get(`http://localhost:3000/items?restaurantId=${restId}&id=${itemId}`)
      if(result.status==200){
        console.log(result.data)
      this.itemName= result.data[0].name,
      this.itemPrice= result.data[0].price,
      this.itemQuantity= result.data[0].qty,
      this.itemDescrip= result.data[0].description,
      this.selectCate= result.data[0].catId
      }else{
        console.error('get item info error')
      }
    ...
    async UpdateItem() {
      ...
      if (this.v$.$error) {
        ...
        if (result.status == 200) {
          ...
          this.SuccessMessage = "Updating Item successfully";
          this.ErrorMessage = "";
          setTimeout(() => {
